...
export async function joinResearchProject(projectId: number, role: string) {
  const { data } = await api.post<ResearchProjectResponse>(`/api/research-projects/${projectId}/join`, { role })
  return data
}
